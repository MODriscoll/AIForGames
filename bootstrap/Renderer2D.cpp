...
void Renderer2D::drawArrowToPoint(float x1, float y1, float x2, float y2, float angle, float tip, float thickness, float depth)
{
	float xDiff = x2 - x1;
	float yDiff = y2 - y1;
	float len = glm::sqrt(xDiff * xDiff + yDiff * yDiff);
	...
	float half = angle / 2.f;
	float prot = rot + half;
	float nrot = rot - half;

	float ax1 = -glm::cos(prot);
	float ay1 = -glm::sin(prot);
	float ax2 = -glm::cos(nrot);
	float ay2 = -glm::sin(nrot);

	rot = glm::atan(ay1, ax1);

	drawSprite(m_nullTexture, x2, y2, tip, thickness, rot, depth, 0.0f, 0.5f);

	rot = glm::atan(ay2, ax2);
...
void Renderer2D::drawArrowFromPoint(float x, float y, float length, float direction, float angle, float tip, float thickness, float depth)
{
	float uvX = m_uvX;
	float uvY = m_uvY;
	float uvW = m_uvW;
	...
	drawSprite(m_nullTexture, x, y, length, thickness, direction, depth, 0.0f, 0.5f);

	float x2 = (glm::cos(direction) * length) + x;
	float y2 = (glm::sin(direction) * length) + y;

	float half = angle / 2.f;
	float prot = direction + half;
	float nrot = direction - half;
